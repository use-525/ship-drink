<?php
$news_list = list_all_news();

if (isset($_GET['id'])) {
	$id = $_GET['id'];
	delete_news($id);
	$_SESSION['news'] = "Xoá tin tức thành công";
...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		<div id="toolbar" class="btn-group">
			<a href="<?= ROOT ?>admin/?page=news&action=add" class="btn btn-success">
				<i class="glyphicon glyphicon-plus"></i> Thêm chương Tin tức
			</a>
		</div>
		<?php if (isset($_SESSION['news'])) : ?>
			<div class="alert alert-success"><?= $_SESSION['news'] ?></div>
		<?php endif; ?>
		<div class="row">
			<div class="col-md-12">
							...
							<tbody>
								<?php foreach ($news_list as $p) : ?>
									<tr>
										...
										<td style="text-align: center"><?= $p['news_title'] ?></td>
										<td style="text-align: center"><img width="130" height="180" src="../images/<?= $p['news_image'] ?>" /></td>
										<td style="text-align: center"><?= $p['news_content'] ?></td>


										<td class="form-group" style="text-align: center">
											<a href="<?= ROOT ?>admin/?page=news&action=update&id=<?= $p['id'] ?>" class="btn btn-primary"><i class="glyphicon glyphicon-pencil"></i></a>
											<a href="<?= ROOT ?>admin/?page=news&id=<?= $p['id'] ?>" onclick="return confirm('Bạn có chắc muốn xoá không?')" class="btn btn-danger"><i class="glyphicon glyphicon-remove"></i></a>
										</td>
									</tr>
								<?php endforeach; ?>
							</tbody>
