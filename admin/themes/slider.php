<?php
$slider_list = list_all_slider();

if (isset($_GET['id'])) {
    $id = $_GET['id'];
    delete_slider($id);
    $_SESSION['slider'] = "Xoá thành công";
...
<body>
    ...
    <div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
        ...
        <div id="toolbar" class="btn-group">
            <a href="<?= ROOT ?>admin/?page=slider&action=add" class="btn btn-success">
                <i class="glyphicon glyphicon-plus"></i> Thêm Slider
            </a>
        </div>
        <?php if (isset($_SESSION['slider'])) : ?>
            <div class="alert alert-success"><?= $_SESSION['slider'] ?></div>
        <?php endif; ?>
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-body">
                        <table data-toolbar="#toolbar" data-toggle="table" class="table">
                            ...
                            <tbody>
                                <?php foreach ($slider_list as $p) : ?>
                                    <tr>
                                        <td style="text-align: center"><?= $p['id'] ?></td>

                                        <td style="text-align: center"><img width="130" height="180" src="../images/<?= $p['slider_image'] ?>" /></td>

                                        <td style="text-align: center"><?= $p['slider_created_at'] ?></td>


                                        <td class="form-group" style="text-align: center">
                                            <a href="<?= ROOT ?>admin/?page=slider&action=update&id=<?= $p['id'] ?>" class="btn btn-primary"><i class="glyphicon glyphicon-pencil"></i></a>
                                            <a href="<?= ROOT ?>admin/?page=slider&id=<?= $p['id'] ?>" onclick="return confirm('Bạn có chắc muốn xoá không?')" class="btn btn-danger"><i class="glyphicon glyphicon-remove"></i></a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
