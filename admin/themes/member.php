<?php
$member_list = list_all_member();
if (isset($_GET['id'])) {
	$id = $_GET['id'];
	delete_member($id);
	$_SESSION['member'] = "Xoá người dùng thành công";
...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		<div id="toolbar" class="btn-group">
			<a href="<?= ROOT ?>admin/?page=member&action=add" class="btn btn-success">
				<i class="glyphicon glyphicon-plus"></i> Thêm thành viên
			</a>
		</div>
		<?php if (isset($_SESSION['member'])) : ?>
			<div class="alert alert-success"><?= $_SESSION['member'] ?></div>
		<?php endif; ?>
		<div class="row">
			<div class="col-lg-12">
						...
						<table data-toolbar="#toolbar" data-toggle="table" class="table">
							...
							<thead>
								<tr>
									...
									<th data-field="name" data-sortable="true" style="text-align:center;">Họ & Tên</th>
									<th data-field="price" data-sortable="true" style="text-align:center;">Email</th>
							...
							<tbody>
								<?php foreach ($member_list as $m) : ?>
									<tr>
										...
										<td style="text-align:center;"><?= $m['member_name'] ?></td>
										<td style="text-align:center;"><?= $m['email'] ?></td>
										<td style="text-align:center;"><span class="label label-danger"><?= ($m['member_level']) ? 'admin' : 'member' ?></span></td>
										<td style="text-align:center;"><?= $m['phone'] ?></td>
										<td style="text-align:center;"><?= $m['address'] ?></td>
										<td class="form-group" style="text-align:center;">
											<a href="<?= ROOT ?>admin/?page=member&action=update&id=<?= $m['id'] ?>" class="btn btn-primary"><i class="glyphicon glyphicon-pencil"></i></a>
											<a href="<?= ROOT ?>admin/?page=member&id=<?= $m['id'] ?>" onclick="return confirm('Bạn có chắc muốn xoá không?')" class="btn btn-danger"><i class="glyphicon glyphicon-remove"></i></a>
										</td>
									</tr>
								<?php endforeach; ?>
							</tbody>
