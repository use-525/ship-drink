...
if (isset($_GET['id'])) {
	$id = $_GET['id'];
	delete_trademark($id);
	$_SESSION['trademark'] = "Xoá nhãn hiệu thành công";
...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		<div id="toolbar" class="btn-group">
			<a href="<?= ROOT ?>admin/?page=trademark&action=add" class="btn btn-success">
				<i class="glyphicon glyphicon-plus"></i> Thêm thương hiệu
			</a>
		</div>
		<?php if (isset($_SESSION['trademark'])) : ?>
			<div class="alert alert-success"><?= $_SESSION['trademark'] ?></div>
		<?php endif; ?>
		<div class="row">
			<div class="col-md-12">
				<div class="panel panel-default">
					<div class="panel-body">
						<table data-toolbar="#toolbar" data-toggle="table" class="table">
							...
							<tbody>
								<?php foreach ($trademark_list as $t) : ?>
									<tr>
										<td style="text-align: center"><?= $t['id'] ?></td>
										<td style="text-align: center"><?= $t['trade_name'] ?></td>
										<td style="text-align: center"><img width="130" height="180" src="../images/<?= $t['trade_image'] ?>" /></td>
										<td class="form-group" style="text-align: center">
											<a href="<?= ROOT ?>admin/?page=trademark&action=update&id=<?= $t['id'] ?>" class="btn btn-primary"><i class="glyphicon glyphicon-pencil"></i></a>
											<a href="<?= ROOT ?>admin/?page=trademark&id=<?= $t['id'] ?>" onclick="return confirm('Bạn có chắc muốn xoá không?')" class="btn btn-danger"><i class="glyphicon glyphicon-remove"></i></a>
										</td>
									</tr>
								<?php endforeach; ?>
							</tbody>
