...
if (isset($_GET['id'])) {
	$id = $_GET['id'];
	delete_category($id);
	$_SESSION['cate'] = "Xoá thư mục thành công!";
...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		<div id="toolbar" class="btn-group">
			<a href="<?= ROOT ?>admin/?page=category&action=add" class="btn btn-success">
				<i class="glyphicon glyphicon-plus"></i> Thêm danh mục
			</a>
		</div>
		<?php if (isset($_SESSION['cate'])) : ?>
			<div class="alert alert-success"><?= $_SESSION['cate'] ?></div>
		<?php endif; ?>
		<div class="row">
			<div class="col-md-12">
							...
							<tbody>
								<?php foreach ($category_list as $c) : ?>
									<tr>
										...
										<td style="text-align:center;"><a href="<?= ROOT ?>admin?page=dashboard_category&id_cate=<?= $c['id'] ?>"><?= $c['cat_name'] ?> </a></td>
										<td style="text-align: center"><img width="130" height="180" src="../images/<?= $c['cat_image'] ?>" /></td>
										<td style="text-align:center;"><?= $c['cat_created_at'] ?></td>
										<td style="text-align:center;"><?= $c['cat_updated_at'] ?></td>
										<td class="form-group" style="text-align:center;">
											<a href="<?= ROOT ?>admin/?page=category&action=update&id=<?= $c['id'] ?>" class="btn btn-primary"><i class="glyphicon glyphicon-pencil"></i></a>
											<a href="<?= ROOT ?>admin/?page=category&id=<?= $c['id'] ?>" onclick="return confirm('Bạn có chắc muốn xoá không?')" class="btn btn-danger"><i class="glyphicon glyphicon-remove"></i></a>
										</td>
									</tr>
								<?php endforeach; ?>
							</tbody>
