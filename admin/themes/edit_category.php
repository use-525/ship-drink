...
if (isset($_POST['btnEdit'])) {
	...
	if ($_FILES['cat_image']['size'] > 0) {
		$ok = true;
		$cat_image = uniqid() . $_FILES['cat_image']['name'];
	} else {
		$cat_image = "";
	}
	if ($ok == true) {
		...
		$cat_image = isset($_POST['hinh']) ? $_POST['hinh'] : '';
	}
	update_category($cat_name, $cat_image, $id);
	if ($ok == true) {
		move_uploaded_file($_FILES['cat_image']['tmp_name'], '../images/' . $cat_image);
	}
	$_SESSION['cate'] = "Cập nhập thư mục thành công!";
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		<div class="row">
			<ol class="breadcrumb">
				...
				<li class="active">Danh mục <?= $category_one['cat_name'] ?></li>
			</ol>
		</div>
		<!--/.row-->
		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">Danh mục:<?= $category_one['cat_name'] ?></h1>
			</div>
		</div>
		<!--/.row-->
		<div class="row">
						...
						<div class="col-md-8">
							...
							<form method="POST" enctype="multipart/form-data">
								<div class="form-group">
									...
									<input type="text" name="cat_name" id="name" class="form-control" placeholder="Nhập tên danh mục" value="<?= $category_one['cat_name'] ?>" required>
								</div>
								<?php if ($category_one['cat_image'] != '') : ?>
									<img src="../images/<?= $category_one['cat_image'] ?>" alt="" width="120">
									<input type="hidden" name="hinh" value="<?= $category_one['cat_image'] ?>">
								<?php endif; ?>
								<div class="form-group">
									<input type="file" name="cat_image" class="form-file-input border" id="<?= $category_one['id'] ?>">
								</div>
